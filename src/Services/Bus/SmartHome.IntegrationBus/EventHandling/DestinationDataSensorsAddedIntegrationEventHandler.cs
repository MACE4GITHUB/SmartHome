...
namespace SmartHome.IntegrationBus.EventHandling
{
        ...
        IIntegrationEventHandler<IntegrationEvent>
    {
        ...
        {
            _logger.LogDebug("Handling integration event from destination (subscribers): {IntegrationEventId} at {AppName} - ({@IntegrationEvent})", @event.EventId, @event.ApplicationName, @event);
